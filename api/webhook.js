module.exports = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      throw new Error('Failed to parse auth response: ' + authText.substring(0, 500));
    }
    ...
    // Add to catalog 232185
    const expertResp = await fetch('https://api.pyrus.com/v4/catalogs/232185/items', {
      method: 'POST',
      headers,
      body: JSON.stringify({
        values: [expertName, location, '', contacts, '']
      })
    });
    const expertText = await expertResp.text();
    console.log('Expert response:', expertText.substring(0, 300));
    const expertResult = JSON.parse(expertText);
    ...
    for (const e of expenses) {
      if (e.price > 0) {
        const resp = await fetch('https://api.pyrus.com/v4/catalogs/232177/items', {
          method: 'POST',
          headers,
          body: JSON.stringify({
            values: [String(e.id), e.name, String(e.price), expertName]
          })
        });
