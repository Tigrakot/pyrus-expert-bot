module.exports = async (req, res) => {
  ...
  try {
    ...
    const expertResp = await fetch('https://api.pyrus.com/v4/catalogs/232185/diff', {
      ...
      headers,
      body: JSON.stringify({
        upsert: [{ values: [expertName, location, String(priceOutOfCity), contacts, ''] }]
      })
    });
    ...
    const expenses = [
      { id: `${expertShort}-1`, name: 'Прием документов', price: priceDoc },
      { id: `${expertShort}-2`, name: 'Осмотр ТС', price: priceOsmotr },
      { id: `${expertShort}-3`, name: 'Выезд по городу', price: priceCity },
      { id: `${expertShort}-4`, name: 'Соглашение', price: priceAgreement }
    ];
